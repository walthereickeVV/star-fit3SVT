...
function initTrainerBooking() {
    ...
    trainerBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            showNotification(`Вы выбрали запись к ${getTrainerName(trainer)}`, 'success');
        });
    });
}
...
function initAnimations() {
    ...
    document.querySelectorAll('.trainer-card, .pricing-category, .advantage-item').forEach(el => {
        observer.observe(el);
    });
}
...
function createUrgencyTimer() {
    ...
    if (pricingSection) {
        const timerExists = document.querySelector('.urgency-timer');
        if (!timerExists) {
